...
from projet_SLR_mirror.test import launch_test, prob_viz
...
class Tuto:
    ...
    def launch_tuto(self):
        ...
        for frame in sequence:
            ...
            frame = list(frame)
            pose_landmarks = [[int(frame[i]*RESOLUTION_X), int(frame[i+1]*RESOLUTION_Y)]
                            for i, _ in enumerate(frame[0:33*4]) if i % 4 == 0]
            face_landmarks = [[int(frame[33*4+i]*RESOLUTION_X), int(frame[33*4+i+1]*RESOLUTION_Y)]
                            for i, _ in enumerate(frame[33*4:33*4+468*3]) if i % 3 == 0]
            # print(face_landmarks)
            left_hands_landmarks = [[int(frame[33*4+468*3+i]*RESOLUTION_X), int(frame[33*4+468*3+i+1]*RESOLUTION_Y)]
                                    for i, _ in enumerate(frame[33*4+468*3: 33*4+468*3+21*3]) if i % 3 == 0]
            right_hands_landmarks = [[int(frame[33*4+468*3+21*3+i]*RESOLUTION_X), int(
                frame[33*4+468*3+21*3+i+1]*RESOLUTION_Y)] for i, _ in enumerate(frame[33*4+468*3+21*3:]) if i % 3 == 0]

            body = Tuto(self.actions,self.action)
